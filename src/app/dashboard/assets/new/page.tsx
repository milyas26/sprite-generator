...
import { PlusCircle, Palette, Boxes } from "lucide-react";
    ...
    <div className="flex gap-5 items-start">
      <div className="flex-1 space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight text-foreground font-heading">New Asset</h1>
            <p className="text-[11px] text-muted-foreground font-mono mt-0.5">Describe a game asset — AI generates a pixel art sprite sheet</p>
          </div>
        </div>
        ...
        <Card className="border-border bg-card overflow-hidden">
          <div className="editor-panel-header flex items-center gap-2 px-4 py-2.5">
            ...
            <CardTitle className="text-foreground font-mono text-xs tracking-wider">ASSET PROMPT</CardTitle>
          </div>
          <CardContent className="p-4">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="category" className="text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-widest">Category</Label>
                <Select value={category} onValueChange={(v) => v && setCategory(v)} disabled={loading}>
                  <SelectTrigger id="category" className="bg-background border-border text-foreground font-mono text-xs h-8">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-popover border-border max-h-64">
                    {ASSET_CATEGORIES.map((cat) => (
                      <SelectItem key={cat.category} value={cat.category} className="text-foreground font-mono text-xs focus:bg-secondary">
                        <span className="flex items-center gap-2">
                          <Boxes className="h-3 w-3 text-muted-foreground" />
                          <span className="font-semibold">{cat.label}</span>
                          <span className="text-muted-foreground font-normal">{cat.description}</span>
                        </span>
                      </SelectItem>
                    ))}
                  </SelectContent>
