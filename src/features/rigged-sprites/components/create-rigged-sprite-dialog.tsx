...
import { Puzzle, UserRound, ChevronDown, ChevronUp, Sparkles } from "lucide-react";
...
interface CreateRiggedSpriteDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function CreateRiggedSpriteDialog({ open, onOpenChange }: CreateRiggedSpriteDialogProps) {
  ...
  const handleReady = useCallback(() => {
    toast.success("Rigged sprite ready! Body parts generated.");
    setTimeout(() => {
          ...
          <form onSubmit={handleSubmit} className="p-4 space-y-3.5">
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <Label htmlFor="rigged-prompt" className="text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-widest">Prompt</Label>
                <button
                  type="button"
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="space-y-1">
                <Label htmlFor="rigged-artStyle" className="text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-widest">Art Style</Label>
                <Select value={artStyle} onValueChange={(v) => v && setArtStyle(v)} disabled={loading}>
                  <SelectTrigger id="rigged-artStyle" size="sm" className="w-full bg-background border-border text-foreground font-mono text-[10px]">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1">
                <Label htmlFor="rigged-detailLevel" className="text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-widest">Detail</Label>
                <Select value={detailLevel} onValueChange={(v) => v && setDetailLevel(v)} disabled={loading}>
                  <SelectTrigger id="rigged-detailLevel" size="sm" className="w-full bg-background border-border text-foreground font-mono text-[10px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-popover border-border">
            ...
            {showDetails && (
              <div className="border-t border-border pt-3 space-y-2.5">
                <div>
                  <Label htmlFor="rigged-name" className="text-[9px] font-mono font-semibold text-muted-foreground uppercase tracking-widest">Name</Label>
                  <Input
                    id="rigged-name"
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  <CompactSelect id="rigged-gender" label="Gender" value={gender} onValueChange={setGender} options={GENDERS} disabled={loading} />
                  <CompactSelect id="rigged-race" label="Race" value={race} onValueChange={setRace} options={RACES} disabled={loading} />
                  <CompactSelect id="rigged-class" label="Class" value={charClass} onValueChange={setCharClass} options={CLASSES} disabled={loading} />
                </div>
                <div className="grid grid-cols-3 gap-1.5">
                  <CompactSelect id="rigged-skinTone" label="Skin" value={skinTone} onValueChange={setSkinTone} options={SKIN_TONES} disabled={loading} />
                  <CompactSelect id="rigged-eyeColor" label="Eyes" value={eyeColor} onValueChange={setEyeColor} options={EYE_COLORS} disabled={loading} />
                  <CompactSelect id="rigged-build" label="Build" value={build} onValueChange={setBuild} options={BUILDS} disabled={loading} />
                </div>
                <div className="grid grid-cols-2 gap-1.5">
                  <CompactSelect id="rigged-height" label="Height" value={height} onValueChange={setHeight} options={HEIGHTS} disabled={loading} />
                  <CompactSelect id="rigged-pov" label="Point of View" value={pov} onValueChange={setPov} options={POVS} disabled={loading} />
                </div>
              </div>
            )}
